...
function buildObjective(variables, objectiveKey, variableNameMap) {
	...
	for (const [varName, coefficients] of Object.entries(variables)) {
		...
		if (score !== void 0 && score !== 0) {
			...
			if (terms.length === 0) terms.push(`${formatNumber(score)} ${escapedName}`);
			else {
				const sign = score >= 0 ? "+ " : "- ";
	...
	if (terms.length === 0) return [" obj: 0"];
	return wrapExpression(" obj: " + terms.join(" "), MAX_LINE_LENGTH);
}
/**
...
function wrapExpression(expression, maxLength) {
	...
	for (const token of tokens) if (currentLine.length === 0) currentLine = token;
	else if (currentLine.length + 1 + token.length <= maxLength) currentLine += " " + token;
	else {
...
function buildConstraints(variables, constraints, variableNameMap) {
	...
	for (const [constraintName, constraint] of Object.entries(constraints)) {
		if (constraint.equal === void 0 && constraint.min === void 0 && constraint.max === void 0) continue;
		const terms = [];
		for (const [varName, coefficients] of Object.entries(variables)) {
			const coeff = coefficients[constraintName];
			if (coeff !== void 0 && coeff !== 0) {
				const escapedName = variableNameMap.get(varName);
				if (terms.length === 0) terms.push(`${formatNumber(coeff)} ${escapedName}`);
				else {
					const sign = coeff >= 0 ? "+ " : "- ";
		...
		if (constraint.equal !== void 0) {
			const fullLine = ` ${`c${constraintIndex++}`}: ${lhs} = ${formatNumber(constraint.equal)}`;
			lines.push(...wrapExpression(fullLine, MAX_LINE_LENGTH));
		} else {
			if (constraint.max !== void 0) {
				const fullLine = ` ${`c${constraintIndex++}`}: ${lhs} <= ${formatNumber(constraint.max)}`;
				lines.push(...wrapExpression(fullLine, MAX_LINE_LENGTH));
			}
			if (constraint.min !== void 0) {
				const fullLine = ` ${`c${constraintIndex++}`}: ${lhs} >= ${formatNumber(constraint.min)}`;
				lines.push(...wrapExpression(fullLine, MAX_LINE_LENGTH));
			}
		}
	}
...
*/
function highsSolutionToLPSolution(highsSolution, reverseNameMap, tolerance = .5) {
	let status;
	...
	for (const [escapedName, column] of Object.entries(highsSolution.Columns)) {
		...
		if (originalName && column.Primal >= tolerance) variables.push([originalName, column.Primal]);
	}
	return {
		status,
...
var require_highs = /* @__PURE__ */ __commonJSMin(((exports, module) => {
	...
	var Module = (() => {
		...
		return async function(moduleArg = {}) {
			...
			var ha = console.log.bind(console), x = console.error.bind(console), y, z = !1, A, w = (a) => a.startsWith("file://"), ia, ja, B, C, D, E, F, ka, la = !1;
			function ma() {
				var a = G.buffer;
				B = new Int8Array(a);
				D = new Int16Array(a);
			...
			var xa = 0, ya = 0, K = () => {
				...
				if (k) {
					var a = require___vite_browser_external();
			...
			var Oa = {
				...
				L: {
					...
					write(a, b, c, d, e, f) {
						...
						if (b.subarray && (!a.M || a.M.subarray)) {
							...
							if (e + d <= a.O) return a.M.set(b.subarray(c, c + d), e), d;
						}
						f = e + d;
				...
				b && (c |= 146);
				return c;
			}, Va = null, Wa = {}, R = [], Xa = 1, Q = null, Ya = !1, Za = !0, $a = {}, N = class {
				constructor(a) {
				...
				constructor(a, b, c, d) {
					...
					_defineProperty(this, "aa", 365);
					_defineProperty(this, "da", 146);
			...
			function V(a, b, c = 438) {
				...
				if (b & 512 && !f) {
					e = d;
					e = "string" == typeof e ? S(e, { fa: !0 }).node : e;
				...
				m.L.open && m.L.open(m);
				f && (c &= 511, d = "string" == typeof d ? S(d, { fa: !0 }).node : d, hb(d, {
			...
			var Db, Eb, Fb, Gb, Hb, Bb, zb, Ab, G, Ib = {
				a: (a, b, c) => {
					var d = new wa(a);
					F[d.$ + 16 >> 2] = 0;
					F[d.$ + 4 >> 2] = b;
				...
				d: function(a, b, c) {
					...
					try {
						...
						switch (b) {
							...
							case 4: return e = K(), d.flags |= e, 0;
							case 12: return e = K(), D[e + 0 >> 1] = 2, 0;
							case 13:
							case 14: return 0;
						}
						return -28;
			...
			const Kb = g.cwrap("Highs_setIntOptionValue", "number", [
				"number",
				"string",
				"number"
			]), Lb = g.cwrap("Highs_setDoubleOptionValue", "number", [
				"number",
				"string",
				"number"
			]), Nb = g.cwrap("Highs_setStringOptionValue", "number", [
				"number",
				"string",
				"string"
			]), Ob = g.cwrap("Highs_setBoolOptionValue", "number", [
				"number",
				"string",
				"number"
			]);
...
//#endregion
//#region ui/worker/reforge_worker.ts
var highs = null;
var cachedWasmUrl = void 0;
/**
...
async function initHiGHS(wasmUrl) {
	...
	if (wasmUrl) cachedWasmUrl = wasmUrl;
	try {
		const baseUrl = getBaseUrl();
		const locateFile = (file) => {
			if (file.endsWith(".wasm")) return cachedWasmUrl || `${baseUrl}highs.wasm`;
			return `${baseUrl}${file}`;
		};
		const highsModule = await Promise.resolve().then(() => /* @__PURE__ */ __toESM(require_highs(), 1));
		highs = await (highsModule.default || highsModule)({ locateFile });
		return true;
	} catch (error) {
...
async function solveProblem(msg) {
	...
	try {
		if (!await initHiGHS() || !highs) throw new Error("Failed to initialize HiGHS");
		const { lpString, reverseNameMap } = modelToLPFormat(model);
		const highsOptions = { presolve: "on" };
		if (options.timeout) highsOptions["time_limit"] = options.timeout / 1e3;
		if (options.tolerance) {}
		postMsg({
			msg: "solve",
			id,
			solution: highsSolutionToLPSolution(highs.solve(lpString, highsOptions), reverseNameMap, .5)
		});
	} catch (error) {
	...
	switch (msg) {
		...
		case "init":
			postMsg({
				msg: "init",
				id,
				success: await initHiGHS(data.wasmUrl)
			});
			break;
		case "solve": await solveProblem(data);
	}
});
initHiGHS().then(() => {
	postMsg({ msg: "ready" });
