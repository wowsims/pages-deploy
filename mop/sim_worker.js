...
var require_highs = /* @__PURE__ */ __commonJSMin(((exports, module) => {
	...
	var Module = (() => {
		...
		return async function(moduleArg = {}) {
			...
			var FS = {
				...
				doSetAttr(stream, node, attr) {
					...
					try {
						...
						if (e instanceof RangeError) throw new FS.ErrnoError(22);
						throw e;
					}
				},
				chrdev_stream_ops: {
			...
			Module["cwrap"] = cwrap;
			var _Highs_create, _Highs_destroy, _Highs_run, _Highs_getModelStatus, __emscripten_timeout, __emscripten_stack_restore, __emscripten_stack_alloc, _emscripten_stack_get_current, wasmMemory;
			function assignWasmExports(wasmExports) {
				_Highs_create = Module["_Highs_create"] = wasmExports["w"];
				_Highs_destroy = Module["_Highs_destroy"] = wasmExports["x"];
				_Highs_run = Module["_Highs_run"] = wasmExports["y"];
				Module["_Highs_readModel"] = wasmExports["z"];
				Module["_Highs_writeSolution"] = wasmExports["A"];
				Module["_Highs_writeSolutionPretty"] = wasmExports["B"];
				Module["_Highs_setBoolOptionValue"] = wasmExports["C"];
				Module["_Highs_setIntOptionValue"] = wasmExports["D"];
				Module["_Highs_setDoubleOptionValue"] = wasmExports["E"];
				Module["_Highs_setStringOptionValue"] = wasmExports["F"];
				_Highs_getModelStatus = Module["_Highs_getModelStatus"] = wasmExports["G"];
				Module["_Highs_call"] = wasmExports["H"];
			...
			Module.Highs_readModel = Module["cwrap"]("Highs_readModel", "number", ["number", "string"]);
			const Highs_setIntOptionValue = Module["cwrap"]("Highs_setIntOptionValue", "number", [
				"number",
				"string",
				"number"
			]);
			const Highs_setDoubleOptionValue = Module["cwrap"]("Highs_setDoubleOptionValue", "number", [
				"number",
				"string",
				"number"
			]);
			const Highs_setStringOptionValue = Module["cwrap"]("Highs_setStringOptionValue", "number", [
				"number",
				"string",
				"string"
			]);
			const Highs_setBoolOptionValue = Module["cwrap"]("Highs_setBoolOptionValue", "number", [
				"number",
				"string",
				"number"
			]);
			Module.Highs_writeSolutionPretty = Module["cwrap"]("Highs_writeSolutionPretty", "number", ["number", "string"]);
			...
			var _Highs_create, _Highs_run, _Highs_destroy, _Highs_getModelStatus, FS;
			Module["solve"] = function(model_str, highs_options) {
				FS.writeFile(MODEL_FILENAME, model_str);
				const highs = _Highs_create();
				assert_ok(() => Module.Highs_readModel(highs, MODEL_FILENAME), "read LP model (see http://web.mit.edu/lpsolve/doc/CPLEX-format.htm)");
				...
				for (const option_name in options) {
					...
					assert_ok(() => setoption(highs, option_name, option_value), `set option '${option_name}'`);
				}
				assert_ok(() => _Highs_run(highs), "solve the problem");
				const status = MODEL_STATUS_CODES[_Highs_getModelStatus(highs)] || "Unknown";
				assert_ok(() => Module.Highs_writeSolutionPretty(highs, SOLUTION_FILENAME), "write and extract solution");
			...
			function setNumericOption(highs, option_name, option_value) {
				...
				if (result === -1 && option_value === (option_value | 0)) result = Highs_setIntOptionValue(highs, option_name, option_value);
				return result;
			}
			function parseNum(s) {
				if (s === "inf") return 1 / 0;
			...
			function lineToObj(headers, line) {
				...
				for (let idx = 0; idx < values.length; idx++) {
					...
					result[header] = parser ? parser(value) : value;
				}
				return result;
			}
			function parseResult(lines, status) {
				...
				for (var j = i + 2; lines[j] != ""; j++) result["Rows"].push(lineToObj(headers, lines[j]));
			...
			function headersForNonEmptyColumns(headerLine, firstDataLine) {
				return [...headerLine.matchAll(/[^\s]+/g)].filter((match) => firstDataLine[match.index] !== " " || firstDataLine[match.index + match[0].length - 1] !== " ").map((match) => match[0]);
			}
			function assert_ok(fn, action) {
				...
				if (err !== 0 && err !== 1) throw new Error("Unable to " + action + ". HiGHS error " + err);
			}
...
//#endregion
//#region ui/worker/sim_worker.ts
(() => {
	const enosys = () => {
		const err = /* @__PURE__ */ new Error("not implemented");
		err.code = "ENOSYS";
		return err;
	};
	if (!globalThis.fs) {
	...
	if (!globalThis.path) globalThis.path = { resolve(...pathSegments) {
	...
	globalThis.Go = class {
		constructor() {
			...
			const getInt64 = (addr) => {
				return this.mem.getUint32(addr + 0, true) + this.mem.getInt32(addr + 4, true) * 4294967296;
			};
			...
			const storeValue = (addr, v) => {
				...
				if (id === void 0) {
					id = this._idPool.pop();
					if (id === void 0) id = this._values.length;
					this._values[id] = v;
		...
		async run(instance) {
			...
			argvPtrs.push(0);
			Object.keys(this.env).sort().forEach((key) => {
			...
			if (offset >= 12288) throw new Error("total length of command line and environment variables exceeds limit");
...
var unsupportedBulkSimAsync = () => {
	throw new Error("bulkSimAsync is only supported by the HTTP worker.");
};
var highs = null;
var highsInitPromise = null;
async function initHiGHS() {
	if (highs) return;
	if (highsInitPromise) return highsInitPromise;
	highsInitPromise = (async () => {
		const highsModule = await Promise.resolve().then(() => /* @__PURE__ */ __toESM(require_highs(), 1));
		highs = await (highsModule.default ?? highsModule)({ locateFile: (file) => file.endsWith(".wasm") ? "highs.wasm" : file });
	})().catch((err) => {
...
function solveHiGHSLP(lp, timeoutSeconds, mipRelGap) {
	try {
		if (!highs) throw new Error("HiGHS has not been initialized.");
		const options = { presolve: "on" };
		if (timeoutSeconds > 0) options.time_limit = timeoutSeconds;
		if (mipRelGap > 0) options.mip_rel_gap = mipRelGap;
...
var go = new Go();
var inst = null;
var requestWasmModule = () => new Promise((resolve) => {
	const onModule = ({ data }) => {
...
requestWasmModule().then((module) => WebAssembly.instantiate(module, go.importObject)).then(async (instance) => {
	inst = instance;
	await go.run(inst);
